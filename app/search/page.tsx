...
const transformResultsForCards = (results: any, type: string): Array<any> => {
	switch (type) {
		case 'search':
			...
			if (results.knowledgeGraph) {
				cards.push({
					...
					tags: Object.entries(results.knowledgeGraph.attributes || {}).map(([key, value]) => `${key}: ${value}`),
					bgColor: getBgColor(0),
					category: results.knowledgeGraph.type || 'Knowledge Graph',
				});
			}

			// Handle organic results
			return [...cards, ...(results.organic || []).map((result: any, index: number) => ({
				date: new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }),
				title: result.title,
				description: result.snippet,
				tags: result.sitelinks ? result.sitelinks.map((link: any) => link.title) : ['Search Result'],
				bgColor: getBgColor(index + 1),
				category: type,
			}))];

		case 'images':
			return (results.images || []).map((result: any, index: number) => ({
				date: new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }),
				title: result.title,
		...
		case 'places':
			return (results.places || []).map((result: any, index: number) => ({
				date: new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }),
				title: result.title,
				description: `${result.address || ''} ${result.category || ''}`.trim(),
				tags: result.rating ? [`Rating: ${result.rating}`] : [],
				bgColor: getBgColor(index),
				category: type,
			}));

		// Keep other cases as they are, but make them null-safe
		case 'news':
			return (results.news || []).map((result: any, index: number) => ({
  ...
  {/* // Results Area */}
			...
			{isLoading ? (
				...
				<div key={type}>
					...
					<StylishCards items={transformResultsForCards(results, type)} />
				</div>
				))
			) : searchQuery ? (
